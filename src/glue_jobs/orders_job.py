"""Orders ETL job — downloads XLSX from raw S3 via pandas bridge, validates, and upserts into Delta."""
...
from src.lib.schema_definitions import ORDERS_SCHEMA, PARTITION_COLS, PRIMARY_KEYS
...
# Timestamps outside this range are treated as data errors, not schema errors.
_TS_MIN = datetime(2020, 1, 1)
_TS_MAX = datetime(2030, 12, 31)


def _build_rules() -> list:
    """Return the ordered validation rules for the orders dataset."""
    return [
        NotNullRule("order_id"),
        UniqueKeyRule(["order_id"]),
        NotNullRule("user_id"),
        NotNullRule("order_timestamp"),
        PositiveAmountRule("total_amount"),
        TimestampRangeRule("order_timestamp", _TS_MIN, _TS_MAX),
    ]


def _xlsx_to_spark_df(spark: SparkSession, local_path: str) -> DataFrame:
    """Read one orders XLSX file via pandas and cast each column to ORDERS_SCHEMA types.

    pandas infers types from Excel cell formats; Spark cast handles any remaining
    mismatches (e.g., date-only cells read as datetime64, integers with NaN as float64).
    """
...
def _read_orders_xlsx(
    spark: SparkSession,
    raw_bucket: str,
    s3_client=None,
) -> DataFrame:
    """List every orders .xlsx in the raw bucket incoming/ prefix and union into one DataFrame.

    Files are downloaded to /tmp one at a time — Glue worker ephemeral storage
    is 20 GB so a single month of orders data fits comfortably.
    """
...
def run(
    ...
    *,
    ...
    """Execute the orders ETL pipeline; returns a summary dict of counts.

    Pass source_df to bypass S3/XLSX reading — used in tests to inject local DataFrames.
    """
    start_ts = time.time()

    if delta_path is None:
        delta_path = cfg.s3.orders_delta_path
    if quarantine_base_path is None:
        quarantine_base_path = f"s3://{cfg.s3.quarantine_bucket}"

    log.info("Orders job started", extra={"table": TABLE, "environment": cfg.environment})

    # --- Read ---
    if source_df is None:
        source_df = _read_orders_xlsx(spark, cfg.s3.raw_bucket)

    total_records = source_df.count()
    log.info("Source read complete", extra={"total_records": total_records, "table": TABLE})
    ...
    # --- Validate ---
    validation_result = DataQualityValidator(_build_rules(), log).validate(source_df)
    ...
    create_delta_table_if_not_exists(spark, ORDERS_SCHEMA, delta_path, PARTITION_COLS[TABLE])
...
def main() -> None:
    ...
    try:
        ...
        log.error(
            "Unexpected failure in orders job",
